# COWORK DEPLOY PROMPT — davetimhazir.xyz
...
- **Repo:** https://github.com/mfos3c/davetimhazir.xyz — branch `main`, **statik site (build YOK)**.
- **Yapı:** Kökte `index.html`; ayrıca `css/`, `js/`, `assets/`, `templates/`. Build output dizini = **kök (`/`)**.
- **Domain:** `davetimhazir.xyz` — registrar **Hostinger**.
...
- `<PAGES_URL>` — ilk deploy sonrası verilen `*.pages.dev` adresi (örn. `davetimhazir-xyz.pages.dev`).
...
- `git ls-remote https://github.com/mfos3c/davetimhazir.xyz main` ile `main`in remote'ta olduğunu doğrula.
...
- `dash.cloudflare.com` → **(kullanıcı login)** → **Add a site** → `davetimhazir.xyz` → **Free** plan.
...
- `hpanel.hostinger.com` → **(kullanıcı login)** → **Domains** → `davetimhazir.xyz` → **DNS / Nameservers** → **Change nameservers** → **Use custom nameservers**.
...
- Doğrula: `dig +short NS davetimhazir.xyz` → çıktı `*.ns.cloudflare.com` göstermeli.

### 4) Cloudflare Pages projesi oluştur (GitHub'dan)
- Cloudflare → **Workers & Pages** → **Create** → **Pages** → **Connect to Git**.
- GitHub'ı **yetkilendir (OAuth — kullanıcı onayı)**, repo: `mfos3c/davetimhazir.xyz`.
- Build ayarları:
...
- Pages projesi → **Custom domains** → **Set up a custom domain**:
  - `davetimhazir.xyz` → zone Cloudflare'de olduğu için CNAME (flatten) **otomatik** oluşur; **Active** olana kadar bekle.
  - `www.davetimhazir.xyz` → otomatik CNAME → proje.
- **Kanonik yön (www → apex, 301):** Cloudflare → **Rules → Redirect Rules** → yeni kural:
  - Eşleşme: hostname = `www.davetimhazir.xyz`
  - Hedef: `https://davetimhazir.xyz/${http.request.uri.path}` , **301** kalıcı.
...
- `dig +short NS davetimhazir.xyz` → cloudflare nameserver'ları.
- `curl -sI https://davetimhazir.xyz` → `HTTP/2 200` + `server: cloudflare`.
- `curl -sI https://www.davetimhazir.xyz` → `301` → apex.
- `curl -sI http://davetimhazir.xyz` → `301`/`308` → https.
...
## KABUL KRİTERLERİ
- `https://davetimhazir.xyz` Cloudflare üzerinden, **geçerli SSL** ile siteyi serve ediyor.
...
2. Repo köküne `CNAME` dosyası: tek satır `davetimhazir.xyz` → commit + push.
